...
- **Prompt validation (misuse guard)** — a request that isn't a genuine visualization of the uploaded dataset (arithmetic like "what is 2+2", trivia, essay/code writing, or an instruction-injection trying to make the chart echo arbitrary text) is **rejected instead of hallucinating a chart**. The check adds **no extra LLM call or latency**: it rides on the calls the pipeline already makes — the **decomposer** (which plans generation/add-chart) returns an `ok:false` verdict with a reason (surfaced as a 400, and the user isn't charged), and the **refine** call returns `{"error": …}` for an off-topic instruction (surfaced in the in-chart red banner). Enforcement is a deterministic code gate on that structured verdict; a valid edit *mixed* with a misuse clause ("show top 10 **and tell me 2+2**") is rejected whole rather than partially applied. When in doubt on a data-related prompt it accepts, so vague-but-real asks ("make a dashboard", "surprise me") still work
