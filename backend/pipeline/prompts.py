REFINE_SYSTEM = (
    ...
    "- For INTERACTIVE FILTER SLIDERS under the chart — the user says 'add a ... slider', 'let me scrub\n"
    "  through ...', or 'add a minimum/maximum <column> filter' WITHOUT naming a specific cutoff number:\n"
    "  set the 'controls' field to a list of control specs. Three kinds:\n"
    "    • {\"column\": <discrete column, e.g. year>, \"kind\": \"scrub\"} — a single-value SLIDER that\n"
    "      steps through that column's distinct values (one view at a time). Use for ORDERED values\n"
    "      (year/season/round) and 'slider' / 'scrub' wording. If it's a DATE column, add\n"
    "      \"time_unit\": \"year\"|\"month\"|\"day\" so the slider steps through periods, not raw timestamps.\n"
    "    • {\"column\": <discrete column>, \"kind\": \"dropdown\"} — a <select> DROPDOWN to pick one value.\n"
    "      Use for 'dropdown', 'pick from a dropdown', 'select a <col>', or UNORDERED / many-valued\n"
    "      categoricals (country, category, team) where a slider would be awkward. Same slicing as scrub.\n"
    "    • {\"column\": <measure column, e.g. wins>, \"kind\": \"min\"} — a threshold SLIDER hiding entities\n"
    "      BELOW the chosen value. Use for 'add a minimum wins filter', 'minimum sepal width slider'.\n"
    "    • {\"column\": <measure column>, \"kind\": \"max\"} — a threshold SLIDER hiding entities ABOVE the\n"
    "      chosen value. Use for 'add a maximum price filter', 'maximum sepal width slider', 'at most N'.\n"
    "      A min AND a max on the SAME column give a range slider pair. ALWAYS handle a 'maximum <col>\n"
    "      filter' the same way as the matching 'minimum <col> filter' — both are sliders here.\n"
    "    • NETWORK ONLY: a 'minimum/maximum connections filter' (a.k.a. degree, 'nodes with at least N\n"
    "      links') filters node DEGREE, which is not a data column — use the virtual column\n"
    "      {\"column\": \"connections\", \"kind\": \"min\"|\"max\"}. Keep this stable so 'remove the connections\n"
    "      filter' can target it. A network VALUE threshold on a numeric column (e.g. 'constructor result\n"
    "      points filter') filters the node's size, so that column MUST be the z_column (node weight) —\n"
    "      set z_column to it AND add a min/max control on the SAME column.\n"
    "  CRITICAL: a 'minimum/maximum <column> filter' with NO explicit number is a SLIDER (use 'controls',\n"
    "  kind min/max) — do NOT put it in the 'filters' field (a wrong guessed cutoff there empties the chart).\n"
    "  At most TWO value-picker controls (scrub/dropdown combined); any number of 'min'/'max' controls.\n"
    "  A scrub/dropdown column MUST NOT be the x-axis column — filtering the x-axis to one value\n"
    "  collapses the chart (a scrub on a temporal x is the one exception — it windows the axis); pick a DIFFERENT\n"
    "  dimension to scrub (or omit the control if none exists). It should also not be the group column.\n"
    "  For SEPARATE year + month sliders on one date column, emit the column\n"
    ...
    "  NUMERIC THRESHOLD with an EXPLICIT number (e.g. 'only where wins >= 2', 'at least 3 points', 'between\n"
    "  18 and 65'): use \"min\"/\"max\" instead of \"values\" — [{\"column\": \"wins\", \"min\": \"2\"}]. (A minimum/\n"
    "  maximum <column> filter with NO stated number is a SLIDER — use 'controls' above, not this field.)\n"
